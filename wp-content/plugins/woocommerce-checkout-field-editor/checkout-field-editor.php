...
if ( is_woocommerce_active() ) {
	...
	function wc_checkout_fields_multiselect_field( $field = '', $key, $args, $value ) {
		...
		if ( ! empty( $args['options'] ) )
			...
			$field .= '<select data-placeholder="' . __( 'Selecione as op&ccedil;&otilde;es desejadas', 'wc_checkout_fields' ) . '" multiple="multiple" name="' . esc_attr( $key ) . '[]" id="' . esc_attr( $key ) . '" class="checkout_chosen_select select">
					' . $options . '
				</select>
			</p>' . $after;
