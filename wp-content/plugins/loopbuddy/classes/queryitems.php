...
Version History
	1.3.0 - 2012-12-01 - Chris Jean
		...
		Added "Settings Saved..." message.
		Misc code cleanup.
	1.3.1 - 2013-10-28 - Chris Jean
		Fixed logic that caused certain combinations of settings to always break the orderby setting by forcing it to meta_value_num.
*/
...
class loopbuddy_queryitems {
	...
	private function get_keys_to_edit( $query_id = -1 ) {
		...
		if ( $query_id != -1 ) { 
			...
			if ( isset( $this->parent->_options[ 'queries' ][ $_GET[ 'id' ] ][ 'query' ] ) ) {
				$saved_query = $this->parent->_options[ 'queries' ][ $_GET[ 'id' ] ][ 'query' ];
			}
		}
		...
		foreach ( $defaults as $label => &$data ) {
			foreach ( $data as $data_key => $values ) {
				...
				if ( $data_key == 'meta' && isset( $saved_query['meta'] ) ) {
	...
	public function get_query_title() {
		$title = 'Default';
		if ( isset( $_GET[ 'id' ] ) ) {
			if ( isset( $this->parent->_options[ 'queries' ][ $_GET[ 'id' ] ] ) ) {
				$query = $this->parent->_options[ 'queries' ][ $_GET[ 'id' ] ];
	...
	public function output_html() {
		?>
		<form method="post" id="pb_edit_queries" action="<?php echo $this->parent->_selfLink; ?>-queries&action=edit&id=<?php echo esc_js( $_GET['id'] ); ?>">
		<h2><img src="<?php echo esc_url( $this->parent->_pluginURL . "/images/loopbuddy_rings.png" ); ?>" style="vertical-align: -4px;" /> Query Settings for <?php echo esc_html( $this->get_query_title() ); ?></h2>
		
		<?php
			if ( isset( $_POST[ 'save' ] ) && isset( $_POST[ 'loopbuddy' ] ) )
				$this->parent->alert( __( 'Settings saved...', 'it-l10n-loopbuddy' ) );
		?>
