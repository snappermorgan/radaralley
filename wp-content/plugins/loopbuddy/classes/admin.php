...
/*
Code to handle the admin features.
...
class pluginbuddy_loopbuddy_admin {
	...
	function check_for_export() {
		if ( ! empty( $_POST['action'] ) ) {
			if ( 'export_layouts' == $_POST['action'] )
				$this->export( 'layouts' );
			else if ( 'export_queries' == $_POST['action'] )
				$this->export( 'queries' );
		}
	}
	...
	function export( $type ) {
		...
		foreach ( (array) $_POST['items'] as $id )
			$groups[] = call_user_func( array( $this->_parent, $function ), $id );
		
		$export = base64_encode( maybe_serialize( $groups ) );
		$export = trim( $export );
		...
		header( "Content-Disposition: attachment; filename=\"loopbuddy_$type.txt\"" );
	...
	function admin_menu() {
		if ( isset( $this->_parent->_series ) && ( $this->_parent->_series != '' ) ) {
			...
			if ( $found_series === false ) {
				add_menu_page( $this->_parent->_series . ' ' . __( 'Getting Started', 'it-l10n-loopbuddy' ), $this->_parent->_series, apply_filters( 'loopbuddy_capability', 'switch_themes' ), 'pluginbuddy-' . strtolower( $this->_parent->_series ), array(&$this, 'view_gettingstarted'), $this->_parent->_pluginURL.'/images/pluginbuddy.png' );
				add_submenu_page( 'pluginbuddy-' . strtolower( $this->_parent->_series ), $this->_parent->_name.' ' . __( 'Getting Started', 'it-l10n-loopbuddy' ), __( 'Getting Started', 'it-l10n-loopbuddy' ), apply_filters( 'loopbuddy_capability', 'switch_themes' ), 'pluginbuddy-' . strtolower( $this->_parent->_series ), array(&$this, 'view_gettingstarted') );
			}
			...
			add_submenu_page( 'pluginbuddy-' . strtolower( $this->_parent->_series ), $this->_parent->_name, $this->_parent->_name, apply_filters( 'loopbuddy_capability', 'switch_themes' ), $this->_parent->_var.'-settings', array(&$this, 'view_settings'));
		} else { // NOT IN A SERIES!
			// Add main menu (default when clicking top of menu)
			add_menu_page($this->_parent->_name.' ' . __( 'Getting Started', 'it-l10n-loopbuddy' ), $this->_parent->_name, apply_filters( 'loopbuddy_capability', 'switch_themes' ), $this->_parent->_var, array(&$this, 'view_gettingstarted'), $this->_parent->_pluginURL.'/images/loopbuddy16.png');
			// Add sub-menu items (first should match default page above)
			add_submenu_page( $this->_parent->_var, $this->_parent->_name.' ' . __( 'Getting Started', 'it-l10n-loopbuddy' ), __( 'Getting Started', 'it-l10n-loopbuddy' ), apply_filters( 'loopbuddy_capability', 'switch_themes' ), $this->_parent->_var, array(&$this, 'view_gettingstarted'));
			add_submenu_page( $this->_parent->_var, $this->_parent->_name.' ' . __( 'Query Editor', 'it-l10n-loopbuddy' ), __( 'Query Editor', 'it-l10n-loopbuddy' ), apply_filters( 'loopbuddy_capability', 'switch_themes' ), $this->_parent->_var.'-queries', array(&$this, 'view_queries'));
			add_submenu_page( $this->_parent->_var, $this->_parent->_name.' ' . __( 'Layout Editor', 'it-l10n-loopbuddy' ), __( 'Layout Editor', 'it-l10n-loopbuddy' ), apply_filters( 'loopbuddy_capability', 'switch_themes' ), $this->_parent->_var.'-layouts', array(&$this, 'view_layouts'));
			$admin_hook = add_submenu_page( $this->_parent->_var, $this->_parent->_name.' ' . __( 'Settings', 'it-l10n-loopbuddy' ), __( 'Settings', 'it-l10n-loopbuddy' ), apply_filters( 'loopbuddy_capability', 'switch_themes' ), $this->_parent->_var.'-settings', array(&$this, 'view_settings'));
