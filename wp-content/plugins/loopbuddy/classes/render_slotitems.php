...
if ( !class_exists( "pluginbuddy_loopbuddy_render_slotitems" ) ) {
	class pluginbuddy_loopbuddy_render_slotitems {
		...
		function the_content( $rs, $slot_item ) {
			...
			// Store the global more so we can reset before returning
			$global_more = $more;

			// If Layout has text in content_more option of Content tag, set as more text and set global $more to 0
			$more_text = empty( $slot_item['content_more'] ) ? null : esc_attr( $slot_item['content_more'] );
			if ( $more_text )
				$more = 0;
			...
			// Return global $more to its original
			$more = $global_more;
			return $return;
		}
		...
		function wp_link_pages( $rs, $slot_item ) {
			$before = empty( $slot_item[ 'before_text' ] ) ? '' : esc_html( $slot_item[ 'before_text' ] );
			$after = empty( $slot_item[ 'after_text' ] ) ? '' : esc_html( $slot_item[ 'after_text' ] );
			ob_start();
			wp_link_pages( array( 'before' => "<p>$before", 'after' => "</p>" ) );
			$return = ob_get_clean();
			$return = pluginbuddy_loopbuddy_render_slotitems::get_output( $return, $rs, $slot_item, true );
			return $return;
		} //end wp_link_pages
		function shortcode( $rs, $slot_item ) {
			$return = do_shortcode( stripslashes($slot_item['shortcode']) );
		...
		function wp_get_attachment_image( $rs, $slot_item ) {
			...
			$return = $before . wp_get_attachment_image( $rs->ID, $slot_item['image_size'] ) . $after;
			$return = pluginbuddy_loopbuddy_render_slotitems::get_output( $return, $rs, $slot_item, true );
			return $return;	
		}
