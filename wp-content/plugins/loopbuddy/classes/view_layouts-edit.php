...
Version History
	1.3.0 - 2012-12-01 - Chris Jean
		...
		Misc code cleanup.
		Changed "edit" query variable to "id".
	1.3.1 - 2012-12-12 - Chris Jean
		Fixed background gradients on WordPress 3.5.
		Fixed issue with drag-and-drop not working on WordPress 3.5.
*/
...
<div class="postbox-container" style="width:60%;">
	...
	$layout = &$this->_parent->get_layout( $_GET['id'] );
			...
			jQuery('.loop_slot').sortable({
				...
				update: function(e,ui) {
					...
					if (this === ui.item.parent()[0]) {
						...
						jQuery.ajax({
							type: 'POST',
							url: '<?php echo admin_url('admin-ajax.php'); ?>?action=pb_loopbuddy_savelayout&group=<?php echo htmlentities( $_GET['id'] ); ?>',
...
//				revert: true,
			...
			jQuery('.loop_action').live('click',function() {
				tb_show( jQuery(this).attr('title'), '<?php echo admin_url('admin-ajax.php'); ?>?action=pb_loopbuddy_editslotitem&id=' + jQuery(this).parent().parent().attr('id').substring(7) + '&slot=' + jQuery(this).parent().parent().parent().attr('id').substring(7) + '&group=<?php echo htmlentities( $_GET['id'] ); ?>&slot=' + jQuery(this).closest('.loop_slot').attr('title'), null );
				return false;
			});
			...
			jQuery('.loop_delete').live('click',function() {
				...
				jQuery( "#pbloop-" + slot ).fadeOut( 'fast', function() { 
					...
					var group = '<?php echo esc_js( htmlentities( $_GET[ 'id' ] ) ); ?>';
	...
	<style type="text/css">
		...
		.loop_item, .loop_item_placed {
			...
			background:url("<?php echo $this->_pluginURL; ?>/images/gray-grad.png") repeat-x scroll left top #DFDFDF;
	...
	<div class="metabox-holder">
		<div class="meta-box-sortables">
			...
			<div id="breadcrumbslike" class="postbox">
				...
				<h3 class="hndle" style="height: 16px;">
					<span style="font-size: 15px;">
						<?php esc_html_e( 'Editing', 'it-l10n-loopbuddy' ); ?> " <?php echo $layout['title']; ?>" <?php $this->tip( __( 'Drag and drop items from the right menu into the `slots` below. You can then click on the gear icon for each item for additional configuration options.', 'it-l10n-loopbuddy' ) ); echo ' '; ?>
					</span>
					<div style="float: right;">
						<span id="pb_saving" style="display: none;">
							<img src="<?php echo $this->_pluginURL; ?>/images/loading.gif" alt="<?php esc_attr_e( 'Saving...', 'it-l10n-loopbuddy' ); ?>" title="<?php esc_attr_e( 'Saving...', 'it-l10n-loopbuddy' ); ?>" style="cursor: default;" width="16" height="16" />
						</span>
					</div>
				</h3>
			...
			-->
					...
					<form method="post" action="<?php echo $this->_selfLink; ?>-layouts&action=edit&id=<?php echo htmlentities( $_GET['id'] ); ?>">
						...
						<input type="hidden" name="savepoint" value="layouts#<?php echo $_GET['id']; ?>" />
