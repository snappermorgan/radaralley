...
//detect mobile
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent)) { window.isMobile = true; document.addEventListener('DOMContentLoaded', function(){ document.body.className += " mobile"; });  } else { window.isMobile = false; }

if (/Android|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent)) { window.isIphone = true; document.addEventListener('DOMContentLoaded', function(){ document.body.className += " iphone"; });  } else { window.isIphone = false; }
...
$(function() {
  var urls = {
    cover: 'ui-kit-cover/js/edit.js',
    image: 'ui-kit-image/js/edit.js',
    text: 'ui-kit-text/js/edit.js',
    feature: 'ui-kit-feature/js/edit.js',
    grid: 'ui-kit-grid/js/edit.js',
    menu: 'ui-kit-menu/js/edit.js',
    footer: 'ui-kit-footer/js/edit.js',
    subscribe: 'ui-kit-subscribe/js/edit.js'
  };


  _.each(_.keys(urls), function(key){
    urls[key] = pluginUrl + '/templates/' + urls[key];
  });

  window.scriptsUrls = urls;
  window.cahcedJS = {};
  var loadingQueue = [];
  window.loadExternalScript = function(el, scrUrl, edit) {
    (function(sel, url) {
      if (window.cahcedJS[scrUrl]) {
        setTimeout(function(){
          try {
            eval(window.cahcedJS[scrUrl])(el, edit);
          } catch(err){}
        }, 0);
        return;
      }

      if (window.cahcedJS[scrUrl] === 0) {
        loadingQueue.push({el: el, url: scrUrl, edit: edit});
        return;
      }

      window.cahcedJS[scrUrl] = 0;
      loadingQueue.push({el: el, url: scrUrl, edit: edit});

      $.ajax({
        url: url,
      }).success(function(script) {
        window.cahcedJS[url] = script;
        var removed = _.remove(loadingQueue, function(item) {
          return item.url == url;
        });

        _.each(removed, function(item) {
          try {
            eval(script)(item.el, item.edit);
          } catch (err) {}
        });
      });
    }(el, scrUrl));
  ...
  var blocks = [];
  $('[component-id]').each(function(index, element) {
      var el = $(element);
  ...
  for (var i = 0; i < blocks.length; i++) {
    var scrUrl = scriptsUrls[blocks[i]];
